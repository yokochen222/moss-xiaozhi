...
void MossSpiLcdDisplay::PauseBackgroundAnimation(bool wait_bus) {
    {
        std::lock_guard<std::mutex> lock(face_ui_mutex_);
        if (face_track_mode_) {
            return;
        ...
        ESP_LOGI(TAG, "Pause code scroll for camera");
        moss_splash::pause_code_scroll_loop();
        scroll_paused_ = true;
    }
...
void MossSpiLcdDisplay::YieldLcdForDvp() {
    moss_splash::pause_code_scroll_loop();
    vTaskDelay(pdMS_TO_TICKS(60));
}

void MossSpiLcdDisplay::StopLcdForTracking() {
    ESP_LOGI(TAG, "Stop code scroll for face-track DVP");
    moss_splash::stop_code_scroll_loop();
    if (!moss_splash::wait_code_scroll_stopped(1500)) {
        ESP_LOGW(TAG, "Code scroll still running after 1500ms; DVP init may fail");
    }
}

void MossSpiLcdDisplay::UnpauseLcdAfterFailedDvp() {
    {
        std::lock_guard<std::mutex> lock(face_ui_mutex_);
        if (face_track_mode_) {
            return;
