...
static void draw_ascii_char(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_top,
                            unsigned char c, uint16_t color);
static void draw_cjk_glyph(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_baseline,
...
static std::atomic<uint32_t> s_render_interval_ms{120};
...
static void DrawPanelBitmapChunked(esp_lcd_panel_handle_t panel, int x0, int y0, int x1, int y1,
    ...
    for (int row = 0; row < h; row += kLcdBounceRows) {
        ...
        LcdWaitIdle();
        // INT WDT is 300ms. Yield so IDLE on this core is fed between SPI bursts.
        vTaskDelay(1);
        if (err != ESP_OK) {
...
static void draw_code_line(uint16_t* dst, int dst_w, int dst_h, int code_x, int y_baseline,
                           const char* line, int char_count) {
    if (line == nullptr || char_count <= 0)
        return;
    int i = 0;
    int x = code_x;
    int drawn = 0;
...
public:
    ...
    static void start_loop_task(const LoopConfig& cfg) {
        ...
        s_task_caps = true;
        BaseType_t ok = xTaskCreateWithCaps(loop_task, "code_scroll", 8192, ctx, 1, &s_task,
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (ok != pdPASS) {
            s_task_caps = false;
            ok = xTaskCreate(loop_task, "code_scroll", 8192, ctx, 1, &s_task);
        }
        if (ok != pdPASS) {
    ...
    static bool wait_stopped(int timeout_ms) {
    ...
    static void loop_task(void* arg) {
        ...
        while (!s_stop_requested) {
            ...
            if (elapsed_char_us >= code_scroll_cfg::TICK_INTERVAL_MS * 1000) {
                last_char_tick_us =
                    now_us - (elapsed_char_us % (code_scroll_cfg::TICK_INTERVAL_MS * 1000));
                ...
                const uint32_t render_ms = s_render_interval_ms.load(std::memory_order_relaxed);
                if (elapsed_render_us >= (int64_t)render_ms * 1000) {
                    last_render_tick_us =
                        now_us - (elapsed_render_us % ((int64_t)render_ms * 1000));
                    ...
                    if (cfg.dialog_state_ptr != nullptr) {
                        ...
                        if (dialog_snap.active && dialog_snap.title[0] != '\0' &&
                            ...
                            if (dialog_snap.active) {
                                ...
                                draw_dialog_box(render_buf, pw, ph, bx, by, bw, bh,
                                                dialog_snap.color, dialog_snap.title,
...
void set_code_scroll_render_interval_ms(int ms) {
    ...
    s_render_interval_ms.store(static_cast<uint32_t>(ms), std::memory_order_relaxed);
}
...
// 前向声明
static int draw_text(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_baseline, const char* s,
                     uint16_t color);
...
static void fill_signal_bg(uint16_t* dst, int dst_w, int dst_h, int box_x, int box_y, int box_w,
                           int box_h) {
...
static void draw_dialog_box(uint16_t* dst, int dst_w, int dst_h, int box_x, int box_y, int box_w,
    ...
    auto plot = [&](int x, int y, uint16_t c) { plot_rgb565(dst, dst_w, dst_h, x, y, c); };
...
static const uint8_t kAscii5x7[96][5] = {
    ...
    {0x7F, 0x41, 0x41, 0x22, 0x1C}, {0x7F, 0x49, 0x49, 0x49, 0x41}, {0x7E, 0x09, 0x09, 0x01, 0x02},
...
static const uint8_t* get_packed_bitmap(const lv_font_glyph_dsc_t* g_dsc) {
    ...
    if (gid == 0)
        return nullptr;  // lvgl convention
...
static void draw_cjk_glyph(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_baseline,
                           const lv_font_glyph_dsc_t* g_dsc, uint16_t color) {
    if (g_dsc == nullptr || g_dsc->box_w <= 0 || g_dsc->box_h <= 0)
        return;
...
static void draw_packed_1bpp(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_baseline,
                             const uint8_t* packed, int box_w, int box_h, int ofs_x, int ofs_y,
                             uint16_t color) {
    if (packed == nullptr || box_w <= 0 || box_h <= 0)
        return;
    uint16_t be = (uint16_t)((color >> 8) | (color << 8));
...
static int draw_codepoint(uint16_t* dst, int dst_w, int dst_h, int x_left, int y_baseline,
                          uint32_t cp, uint16_t color) {
    if (cp == 0x53F7) {
        draw_packed_1bpp(dst, dst_w, dst_h, x_left, y_baseline, kHaoBitmap, kHaoBoxW, kHaoBoxH,
                         kHaoOfsX, kHaoOfsY, color);
        return kHaoAdv;
    }
    if (cp < 128) {
...
static bool should_show_dialog(uint8_t priority) { return priority >= kPriorityInfo; }
...
static void build_centered(const uint16_t* src, int frame_w, int frame_h, uint16_t* dst, int dst_w,
                           int dst_h) {
...
void set_dialog_state(DialogState* state, bool active, const char* title, uint16_t color,
                      const char* body) {
    set_dialog_state(state, active, title, color, moss_splash::kPriorityInfo, body,
...
void play_splash(esp_lcd_panel_handle_t panel, const uint8_t* bin_base, size_t bin_size,
    ...
    if (!find_asset_in_bin(bin_base, bin_size, asset_name, asset)) {
        ESP_LOGE(TAG, "Asset '%s' not found in MMAP bin (%zu bytes)", asset_name, bin_size);
        return;
    }
    ...
    uint16_t* frame_buf =
        (uint16_t*)heap_caps_aligned_alloc(16, frame_buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!frame_buf) {
        frame_buf = (uint16_t*)heap_caps_aligned_alloc(16, frame_buf_size,
    ...
    uint16_t* out_buf =
        (uint16_t*)heap_caps_aligned_alloc(16, out_buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!out_buf) {
        out_buf = (uint16_t*)heap_caps_aligned_alloc(16, out_buf_size,
                                                     MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    }
    if (!out_buf) {
    ...
    ESP_LOGI(TAG, "Playing %d frames at %d fps (%d ms) on %dx%d panel [NO-TEXT MODE]", total_frames,
             fps, total_frames * frame_delay_ms, panel_w, panel_h);
...
void draw_face_track_hud(esp_lcd_panel_handle_t panel, uint16_t* buf, int panel_w, int panel_h,
                         ...
                         uint32_t detect_ms, bool gimbal_moving, const uint16_t* preview_rgb565,
                         int preview_w, int preview_h) {
    if (panel == nullptr || buf == nullptr || panel_w <= 0 || panel_h <= 0) {
        return;
    }
    if (frame_w <= 0) {
        frame_w = 320;
    ...
    if (preview_rgb565 && preview_w > 0 && preview_h > 0) {
        if (preview_w == panel_w && preview_h == panel_h) {
            std::memcpy(buf, preview_rgb565, (size_t)panel_w * panel_h * sizeof(uint16_t));
        } else {
            for (int y = 0; y < panel_h; ++y) {
                const int sy = y * preview_h / panel_h;
                for (int x = 0; x < panel_w; ++x) {
                    const int sx = x * preview_w / panel_w;
                    buf[y * panel_w + x] = preview_rgb565[sy * preview_w + sx];
                }
            }
        }
    } else {
        std::memset(buf, 0, (size_t)panel_w * panel_h * sizeof(uint16_t));
    }
    ...
    auto plot = [&](int x, int y, uint16_t c) {
        ...
        buf[y * panel_w + x] = (uint16_t)((c >> 8) | (c << 8));
    };
    auto hline = [&](int x0, int x1, int y, uint16_t c) {
        if (x0 > x1) {
            std::swap(x0, x1);
        }
        for (int x = x0; x <= x1; ++x) {
    ...
    auto vline = [&](int x, int y0, int y1, uint16_t c) {
        if (y0 > y1) {
            std::swap(y0, y1);
        }
        for (int y = y0; y <= y1; ++y) {
    ...
    // Crosshair at panel center.
    const int cx = panel_w / 2;
    const int cy = panel_h / 2;
    hline(cx - 6, cx + 6, cy, kWhite);
    vline(cx, cy - 6, cy + 6, kWhite);

    if (has_face) {
        const int sx1 = box_x1 * panel_w / frame_w;
        const int sy1 = box_y1 * panel_h / frame_h;
        const int sx2 = box_x2 * panel_w / frame_w;
        const int sy2 = box_y2 * panel_h / frame_h;
        hline(sx1, sx2, sy1, kGreen);
        hline(sx1, sx2, sy2, kGreen);
        vline(sx1, sy1, sy2, kGreen);
        vline(sx2, sy1, sy2, kGreen);
        // Corner marks thicker
        hline(sx1, sx1 + 4, sy1, kGreen);
        hline(sx2 - 4, sx2, sy1, kGreen);
        hline(sx1, sx1 + 4, sy2, kGreen);
        hline(sx2 - 4, sx2, sy2, kGreen);
    }
    ...
    char line3[48];
    if (has_face) {
        snprintf(line1, sizeof(line1), "FACE %d  %dx%d", faces, face_w, face_h);
        snprintf(line2, sizeof(line2), "ERR %d,%d", err_x, err_y);
    } else {
        snprintf(line1, sizeof(line1), "NO FACE");
        snprintf(line2, sizeof(line2), "lost...");
    }
    snprintf(line3, sizeof(line3), "%ums %s", (unsigned)detect_ms, gimbal_moving ? "MOVE" : "IDLE");

    // Text with black shadow for readability on live preview.
    auto text_shadow = [&](int x, int y, const char* s, uint16_t color) {
        draw_text(buf, panel_w, panel_h, x + 1, y + 1, s, 0x0000);
        draw_text(buf, panel_w, panel_h, x, y, s, color);
    };
    text_shadow(2, 10, "TRACK", kCyan);
    text_shadow(2, 22, line1, has_face ? kGreen : kRed);
