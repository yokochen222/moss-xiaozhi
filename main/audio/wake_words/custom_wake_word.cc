...
bool CustomWakeWord::Initialize(AudioCodec* codec) {
    ...
    esp_mn_commands_add(1, "ni hao mo si");  // 添加自定义唤醒词作为命令词
    esp_mn_commands_add(2, "ni hao mao si");  // 添加自定义唤醒词作为命令词
    esp_mn_commands_add(3, "xiao tai xian");  // 添加自定义唤醒词作为命令词
...
void CustomWakeWord::AudioDetectionTask() {
    ...
    while (true) {
        ...
        if (mn_state == ESP_MN_STATE_DETECTING) {
            ...
            if (mn_result->command_id[0] == 1 || mn_result->prob[0] >= 0.32) {  // 自定义唤醒词
