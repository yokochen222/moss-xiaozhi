...
class MossCameraVoiceGuard {
public:
    MossCameraVoiceGuard() {
        ...
        if (codec != nullptr) {
            input_was_on_ = codec->input_enabled();
            if (input_was_on_) {
...
#endif
                        // Lower priority only for DVP grab. Explain HTTP at prio 1 starves
                        // audio/LCD.
        ...
        for (auto& argument : arguments) {
            ...
            if (cJSON_IsObject(tool_arguments)) {
                ...
                if (argument.type() == kPropertyTypeBoolean && cJSON_IsBool(value)) {
                    ...
                    found = true;
                } else if (argument.type() == kPropertyTypeInteger && cJSON_IsString(value) &&
                           value->valuestring) {
                    char* end = nullptr;
                    const long parsed = strtol(value->valuestring, &end, 10);
                    if (end != value->valuestring && end && *end == '\0' && parsed >= INT_MIN &&
                        parsed <= INT_MAX) {
                        argument.set_value<int>(static_cast<int>(parsed));
                        found = true;
                    }
