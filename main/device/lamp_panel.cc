...
bool LampPanelDevice::TurnOnPanelLeds() {
    ...
    if (!ApplyOutputs())
        return false;
    ESP_LOGI(TAG, "舷灯已点亮 brightness=%u%%", panel_brightness_);
    return true;
}
...
bool LampPanelDevice::TurnOnBottomLed() {
    ...
    if (!ApplyOutputs())
        return false;
    ESP_LOGI(TAG, "锚灯已点亮 fixed=%u%%", kBottomFixedPwmPercent);
    return true;
}
...
bool LampPanelDevice::TurnOnAll() {
    ...
    if (!ApplyOutputs())
        return false;
    ESP_LOGI(TAG, "舷灯与锚灯全部点亮 panel=%u%% bottom=%u%%", panel_brightness_,
