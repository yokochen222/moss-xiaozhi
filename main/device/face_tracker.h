...
private:
    ...
    bool EnsureDetectBuffer(int w, int h);
    ...
    bool task_caps_ = false;  // stack from xTaskCreatePinnedToCoreWithCaps
    ...
    // Gimbal kHSign=-1 makes MCP/UI left correct; invert tracker h_dir so a face
    // on the right of the frame still pans toward the face.
    static constexpr int kTrackHSign = -1;
    static constexpr uint16_t kMinStepDelayMs = 4;
    static constexpr uint16_t kMaxStepDelayMs = 8;
