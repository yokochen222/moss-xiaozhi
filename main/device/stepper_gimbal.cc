...
// MCP/UI "left" must pan the camera left. This board's M2 wiring is reversed vs that.
static constexpr int kHSign = -1;
...
bool StepperGimbalDevice::StartMoveTask(int16_t h_steps, int16_t v_steps, StepMode mode,
    ...
    if (result != pdPASS) {
        ESP_LOGW(TAG, "Pinned move task failed, retry without affinity");
        result = xTaskCreate(MoveTask, "GimbalStep", 3072, this, 2, &task_handle_);
    }
    if (result != pdPASS) {
...
bool StepperGimbalDevice::EnsureFollowTask(StepMode mode) {
    ...
    follow_mode_ = true;
    moving_ = true;
    follow_task_caps_ = true;
    BaseType_t result = xTaskCreatePinnedToCoreWithCaps(
        FollowTask, "GimbalFollow", 3072, this, 1, &task_handle_, 1,
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (result != pdPASS) {
        follow_task_caps_ = false;
        ESP_LOGW(TAG, "Pinned follow task (PSRAM) failed, retry internal");
        result = xTaskCreatePinnedToCore(FollowTask, "GimbalFollow", 3072, this, 1, &task_handle_, 1);
    }
    if (result != pdPASS) {
        follow_task_caps_ = false;
        ESP_LOGW(TAG, "Pinned follow task failed, retry without affinity");
        result = xTaskCreate(FollowTask, "GimbalFollow", 3072, this, 1, &task_handle_);
    }
    if (result != pdPASS) {
...
bool StepperGimbalDevice::SetFollowRates(int8_t h_dir, int8_t v_dir, uint16_t delay_ms,
    ...
    ESP_LOGI(TAG, "Follow rates h=%d v=%d delay=%ums", static_cast<int>(h_dir),
             static_cast<int>(v_dir), static_cast<unsigned>(delay_ms));
    return true;
}
...
bool StepperGimbalDevice::MoveAxes(int16_t h_steps, int16_t v_steps, StepMode mode,
    ...
    h_steps = static_cast<int16_t>(kHSign * clamp_axis(h_steps));
...
bool StepperGimbalDevice::Idle() {
    ...
    follow_v_dir_ = 0;
    AllCoilsOffLocked();
    ESP_LOGI(TAG, "Idle (coils off, follow task kept)");
    return true;
}
...
// HZ>=1000：整段 vTaskDelay，最多切 4ms 片。切成 1ms 会被 LCD/WiFi 拉长，观感一卡一卡。
// HZ=100：vTaskDelay 最少约 10ms，改用 esp_timer 精确定时。
static bool StepDelayMsInterruptible(uint16_t delay_ms, volatile bool* stop_requested) {
    if (configTICK_RATE_HZ >= 1000) {
        TickType_t remaining = pdMS_TO_TICKS(delay_ms < 1 ? 1 : delay_ms);
        if (remaining < 1) {
            remaining = 1;
        }
        const TickType_t max_slice = pdMS_TO_TICKS(4);
        while (remaining > 0) {
            if (stop_requested && *stop_requested) {
                return true;
            }
            TickType_t slice = (max_slice > 0 && remaining > max_slice) ? max_slice : remaining;
            vTaskDelay(slice);
            remaining = remaining > slice ? static_cast<TickType_t>(remaining - slice) : 0;
        }
...
void StepperGimbalDevice::FollowTask(void* arg) {
    ...
    while (!self->stop_requested_ && self->follow_mode_) {
        ...
        const uint8_t out = static_cast<uint8_t>(h_mask | v_mask);
        {
            ...
            if (self->follow_h_dir_ == 0 && self->follow_v_dir_ == 0) {
                self->AllCoilsOffLocked();
                coils_idle = true;
            } else {
                self->shift_register_->SetOutputs(out);
    ...
    bool caps = false;
    {
        ...
        self->stop_requested_ = false;
        self->task_handle_ = nullptr;
        caps = self->follow_task_caps_;
        self->follow_task_caps_ = false;
    }
