...
class MossBoardIdentityTests(unittest.TestCase):
    ...
    def test_wake_and_mic_defaults_are_shared(self):
        ...
        self.assertIn("AFE is always MR", shared)
    ...
    def test_both_boards_pass_shared_codec_gains_not_zero_reference(self):
        ...
        pcb = (ROOT / "main/boards/moss/moss-pcb-v1/moss_pcb_v1_board.cc").read_text(encoding="utf-8")
...
class MossCmakeSourceIsolationTests(unittest.TestCase):
    ...
    def test_pcb_v1_compiles_eye_and_bar_not_panel_or_motor(self):
        family_start = self.cmake.find("if(CONFIG_BOARD_FAMILY_MOSS)")
        family = self.cmake[family_start:]
        oled_start = family.find("if(CONFIG_BOARD_MOSS_OLED)")
        self.assertGreaterEqual(oled_start, 0)
        oled = family[oled_start : family.find("endif()", oled_start)]
        self.assertIn("device/lamp_bar.cc", oled)
        self.assertIn("device/lamp_eye.cc", oled)
        self.assertNotIn("device/lamp_panel.cc", oled)
        self.assertNotIn("device/eye_motor.cc", oled)
        self.assertNotIn("camera_handlers.cc", oled)
        self.assertNotIn("stepper_gimbal.cc", oled)
        self.assertIn("AND NOT CONFIG_BOARD_MOSS_OLED", self.cmake)
        self.assertIn("font_noto_sans_basic_14_1", self.cmake)
...
class MossBargeInTests(unittest.TestCase):
    ...
    def test_realtime_tts_keeps_aec_uplink_like_xiaozhi(self):
        src = (ROOT / "main/application.cc").read_text(encoding="utf-8")
        start = src.find("if (bits & MAIN_EVENT_SEND_AUDIO)")
        self.assertGreater(start, 0)
        body = src[start : src.find("if (bits & MAIN_EVENT_WAKE_WORD_DETECTED)", start)]
        self.assertIn("SendUplinkFromQueue()", body)
        self.assertNotIn("pending_listening_start_", body)
        self.assertNotIn("kAecOnDeviceSide", body)
        self.assertNotIn("HoldSpeakingUplink()", body)
        self.assertNotIn("UpdateSpeakingBargeIn()", body)
        service = (ROOT / "main/audio/audio_service.cc").read_text(encoding="utf-8")
        self.assertIn("if (!device_aec_enabled_)", service)
        self.assertNotIn("FillSoftwareAecReference", service)
        speaking = src[
            src.find("case kDeviceStateSpeaking:") : src.find("case kDeviceStateWifiConfiguring:")
        ]
        self.assertIn("BeginSpeakingCapture()", speaking)
        listening = src[
            src.find("case kDeviceStateListening:") : src.find("case kDeviceStateSpeaking:")
        ]
        self.assertIn("FlushSpeakingCaptureToSendQueue()", listening)
        self.assertIn("EndSpeakingCapture()", listening)
        self.assertNotIn("pending_listening_start_ = true", listening)
        self.assertIn("StartListeningAudio()", listening)
        self.assertNotIn("kListeningModeAutoStop && !audio_service_.IsPlaybackIdle()", listening)
        self.assertNotIn("while (audio_service_.PopPacketFromSendQueue())", speaking)
        afe = (ROOT / "main/audio/engines/afe_audio_engine.cc").read_text(encoding="utf-8")
        self.assertNotIn("prepend_vad_cache", afe)
        self.assertNotIn("vad_cache", afe)
    ...
    def test_vad_during_tts_uses_silence_arm_then_sustain(self):
        ...
        self.assertIn("kDeviceStateListening", vad)
        self.assertIn("led->OnStateChanged()", vad)

    def test_speaking_start_arms_after_silence_and_keeps_vad(self):
        src = (ROOT / "main/application.cc").read_text(encoding="utf-8")
        speaking = src[
            src.find("case kDeviceStateSpeaking:") : src.find("case kDeviceStateWifiConfiguring:")
        ]
    ...
    def test_barge_in_confirm_is_sustained_vad(self):
        ...
        self.assertNotIn("HasNearEndCapture()", confirm)
        ...
        self.assertIn("IsVadBargeInEnabled()", start_timer)
        self.assertNotIn("IsConfirmedNearEndSpeech()", start_timer)
        self.assertIn("kSpeakingBargeInGuardUs", start_timer)
        self.assertIn("sustain_ms = 200", start_timer)
    ...
    def test_barge_in_resends_listen_start(self):
        ...
        self.assertIn("SendStartListening", start)
        self.assertNotIn("FlushBargeInHold", start)

    def test_afe_uses_ov2640_barge_in_pipeline(self):
        src = (ROOT / "main/audio/audio_service.cc").read_text(encoding="utf-8")
        header = (ROOT / "main/audio/audio_service.h").read_text(encoding="utf-8")
        self.assertIn("speaking_capture_.load", src)
        self.assertIn("TTS residual is the echo floor", src)
        self.assertIn("EchoProfileReady()", src)
        self.assertIn("AEC echo floor learned", src)
        self.assertIn("if (!IsWakeWordRunning())", src)
        self.assertIn(".enable_dtx         = false", header)
        afe = (ROOT / "main/audio/engines/afe_audio_engine.cc").read_text(encoding="utf-8")
        self.assertIn("AFE_TYPE_VC", afe)
        self.assertIn("aec_mode = AEC_MODE_VOIP_HIGH_PERF", afe)
        self.assertIn("aec_nlp_level = AEC_NLP_LEVEL_VERYAGGR", afe)
        self.assertNotIn("AEC_MODE_FD_HIGH_PERF", afe)
        self.assertNotIn("AFE_TYPE_FD", afe)
        self.assertNotIn("AFE_TYPE_SR", afe)
        self.assertIn("vad_mode = VAD_MODE_0", afe)
        self.assertIn("vad_min_noise_ms = 100", afe)
        self.assertNotIn("vad_min_speech_ms = 250", afe)
        self.assertNotIn("vad_delay_ms = 512", afe)
        self.assertNotIn("vad_cache", afe)
        self.assertNotIn("vad_mute_playback = true", afe)
        ...
        box = (ROOT / "main/audio/codecs/box_audio_codec.cc").read_text(encoding="utf-8")
        self.assertIn("MIC1, MIC3, MIC2, MIC4", box)
        self.assertIn("ESP_CODEC_DEV_MAKE_CHANNEL_MASK(1)", box)
        self.assertIn("never MMR", box)
        init = afe[afe.find("if (codec_->input_reference())") : afe.find("print_pipeline")]
        self.assertIn("disable_aec(afe_data_)", init)
...
class MossPcbV1BoardTests(unittest.TestCase):
    def test_oled_not_st7735_and_no_emote_style(self):
        header = (ROOT / "main/boards/moss/moss-pcb-v1/config.h").read_text(encoding="utf-8")
        self.assertIn("#define DISPLAY_WIDTH 128", header)
        self.assertIn("#define DISPLAY_HEIGHT 64", header)
        self.assertIn("GPIO_NUM_48", header)
        self.assertNotIn("DISPLAY_LCD_BOUNCE_ROWS", header)
        board = (ROOT / "main/boards/moss/moss-pcb-v1/moss_pcb_v1_board.cc").read_text(
            encoding="utf-8"
        )
        self.assertIn("OledDisplay", board)
        self.assertIn("esp_lcd_new_panel_ssd1306", board)
        self.assertIn("GetBacklight() override { return nullptr; }", board)
        self.assertNotIn("splash_player", board)
        append = "\n".join(
            json.loads(
                (ROOT / "main/boards/moss/moss-pcb-v1/config.json").read_text(encoding="utf-8")
            )["builds"][0]["sdkconfig_append"]
        )
        self.assertNotIn("USE_EMOTE_MESSAGE_STYLE", append)
        self.assertFalse((ROOT / "main/boards/moss/moss-pcb-v1/splash_player.cc").exists())

    def test_camera_td_uses_old_oled_pins_and_no_panel_motor(self):
        header = (ROOT / "main/boards/moss/moss-camera-td/config.h").read_text(encoding="utf-8")
        self.assertIn("#define DISPLAY_WIDTH 128", header)
        self.assertIn("#define DISPLAY_HEIGHT 64", header)
        self.assertIn("DISPLAY_SDA_PIN GPIO_NUM_7", header)
        self.assertIn("DISPLAY_SCL_PIN GPIO_NUM_6", header)
        self.assertIn("GPIO_NUM_48", header)
        self.assertNotIn("DISPLAY_LCD_BOUNCE_ROWS", header)
        board = (ROOT / "main/boards/moss/moss-camera-td/moss_camera_td_board.cc").read_text(
            encoding="utf-8"
        )
        self.assertIn("MossCameraTdBoard", board)
        self.assertIn("OledDisplay", board)
        self.assertIn("esp_lcd_new_panel_ssd1306", board)
        self.assertNotIn("splash_player", board)
    ...
    def test_hw_rejects_panel_bottom_motor_and_all_only_eye_bar(self):
        src = (ROOT / "main/config/moss_hw.cc").read_text(encoding="utf-8")
        start = src.find("#if CONFIG_BOARD_MOSS_OLED")
        self.assertGreater(start, 0)
        body = src[start : src.find("#else", start)]
        self.assertIn('device == "panel"', body)
        self.assertIn('device == "bottom"', body)
        self.assertIn('device == "motor"', body)
        self.assertIn("unsupported on this board", body)
        self.assertIn("eye_on()", body)
        self.assertIn("StartFlow()", body)
        self.assertNotIn("LampPanelDevice", body)
        self.assertNotIn("EyeMotorDevice", body)

    def test_mcp_panel_and_motor_not_registered(self):
        panel = (ROOT / "main/mcp/tools/lamp_panel.cc").read_text(encoding="utf-8")
        motor = (ROOT / "main/mcp/tools/eye_motor.cc").read_text(encoding="utf-8")
        self.assertIn("#ifndef CONFIG_BOARD_MOSS_OLED", panel)
        self.assertIn("#ifndef CONFIG_BOARD_MOSS_OLED", motor)
        self.assertIn("DECLARE_MCP_TOOL_INSTANCE", panel)
        self.assertIn("DECLARE_MCP_TOOL_INSTANCE", motor)
