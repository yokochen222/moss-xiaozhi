...
class MossCameraPreviewPolicyTests(unittest.TestCase):
    ...
    def test_capture_still_pauses_lcd_for_photo_path(self):
        ...
        self.assertIn("ReleaseSensorKeepJpeg()", capture)
        self.assertIn("ResumeLcdAfterDvp()", capture)
        self.assertIn("CaptureJpegUnderBudgetLocked()", capture)
        fail_resume = slice_between(capture, "if (!ok)", "else if (camera_")
        self.assertIn("ResumeAfterExternalCameraUse()", fail_resume)
        parked = slice_between(capture, "else if (camera_ && !camera_->IsInitialized())", "return ok;")
        self.assertNotIn("ResumeAfterExternalCameraUse()", parked)
        ...
        self.assertIn("CapturedJpegLen", self.board)
